...
export default function Friend({ friendId, name, subtitle, userPicturePath }) {
    ...
    const patchFriend = async () => {
        const response = await fetch(`http://192.168.0.125:3001/users/${_id}/${friendId}`, {
            method: "PATCH",
            headers: {
    ...
    return (
        <FlexBetween>
            ...
            {friendId !== _id && <IconButton onClick={() => patchFriend()} sx={{ backgroundColor: primaryLight, p: "0.6rem" }}>
