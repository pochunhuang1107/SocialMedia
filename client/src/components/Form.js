...
const Form = () => {
    ...
    const register = async (values, onSubmitProps) => {
        ...
        const savedUserResponse = await fetch("http://192.168.0.125:3001/auth/register", {
            method: "POST",
            body: formData,
        });
    ...
    const login = async (values, onSubmitProps) => {
        const loggedInResponse = await fetch("http://192.168.0.125:3001/auth/login", {
            method: "POST",
    ...
    return <Formik onSubmit={handleFormSubmit} initialValues={isLogin ? initialValuesLogin : initialValuesRegister} validationSchema={isLogin ? loginSchema : registerSchema}>
        {({
            values,
            errors,
            touched,
