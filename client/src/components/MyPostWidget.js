import { EditOutlined, DeleteOutlined, AttachFileOutlined, GifBoxOutlined, ImageOutlined, MoreHorizOutlined, MicOutlined } from "@mui/icons-material";
            ...
            {isImage && <Box border={`1px solid ${medium}`} borderRadius="5px" mt="1rem" p="1rem">
                <Dropzone acceptedFiles=".jpg,.jpeg,.png" multiple={false} onDrop={(acceptedFiles) => {
                    ...
                    {({ getRootProps, getInputProps }) => (
                        <FlexBetween>
                            <Box {...getRootProps()} width="100%" border={`2px dashed ${palette.primary.main}`} p="1rem" sx={{ "&:hover": { cursor: "pointer" } }}>
                                <input {...getInputProps()} />
                                {!image ?
                                    <p>Add Image</p>
                                    : <FlexBetween>
